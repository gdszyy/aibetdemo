...
const QuestionsCard: FunctionComponent = () => {
    ...
    const { data, isPending, isError } = useQuery({
        queryKey: ['faq', 'front', 'deposit', FaqDisplayPlaceEnum.Deposit],
        queryFn: () =>
            GetFrontFaqListInterface({
                display_place: FaqDisplayPlaceEnum.Deposit,
                language,
            }),
        staleTime: 5 * 60 * 1000,
    });
    ...
    return (
        <div className="account-card flex flex-col">
            <span className="mb-2 text-auxiliary-md text-filltext-ft-g">{tDeposit('infoPanel.questionsTitle')}</span>

            {isPending ? (
                <div className="flex flex-col gap-2">
                    {Array.from({ length: 5 }, (_, index) => (
                        <div
                            key={`deposit-faq-skeleton-${index + 1}`}
